...
def get_allowed_sheets(all_sheets, role):
    ...
    if role == "Admin":
        return [s for s in all_sheets if "labour" not in normalize(s)]

    elif role == "HR Head":
        return [s for s in all_sheets if "employeeinfo" in normalize(s) or "performance" in normalize(s)]

    elif role == "Finance Head":
        return [s for s in all_sheets if any(k in normalize(s) for k in
            ["finalpayroll", "earnings", "deductions", "tax", "overtime"])]
...
# -----------------------------
    ...
    def generate_pdf():
        ...
        for i, fig in enumerate(st.session_state.charts):
            ...
            fig.write_image(path, engine="kaleido", scale=3)
